...
```js
function loop(initial,length,how,what) {
  for(let i = initial; length(i); i = how(i)){
    what(i);
  }
}
...
function intersection(...arrays) {
  ...
  for(let i = 1; i < arrays.length; i++){
    let second = arrays[i];
    first = first.filter(el => second.includes(el));
  }
  return first;
}
...
function union(...arrays) {
  ...
  for (let i = 1; i < arrays.length; i++){
    second = arrays[i];
    first = first.filter(el => !second.includes(el)).concat(second);
  }
  return first;
}
